    ...
    public function collection(Collection $rows)
    {
    	foreach ($rows as $row) {
		    $this->sheetData[] = [
		    	...
		    	'job_detail'    => isset($row[6])  ? trim($row[6])  : '',
		    	'job_detail_2'  => isset($row[7])  ? trim($row[7])  : '',
		    	'job_detail_3'  => isset($row[8])  ? trim($row[8])  : '',
		    	'job_detail_4'  => isset($row[9])  ? trim($row[9])  : '',
		    	'job_detail_5'  => isset($row[10]) ? trim($row[10]) : '',
		    	'working_place' => isset($row[11]) ? trim($row[11]) : '',
