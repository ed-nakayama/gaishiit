...
<head>
	...
	<div class="mainContentsInner">
		...
		<div class="containerContents">
			...
			@if (strpos($event->person ,Auth::user()->id) !== false)
				{{ html()->form('POST', '/comp/event/change')->id('changeform')->attribute('name', 'changeform')->open() }}
				{{ html()->hidden('event_id', old('event_id' ,$event->id)) }}
			@endif
...
<hr>
			...
			{{ html()->form('GET', '/comp/event/edit')->id('editform')->attribute('name', 'editform')->open() }}
			{{ html()->hidden('event_id', old('event_id' ,$event->id)) }}
			<section class="secContents-mb">
			...
			{{ html()->form('POST', '/comp/event/aprove')->id('aproveform')->attribute('name', 'aproveform')->open() }}
			{{ html()->hidden('event_id', old('event_id' ,$event->id)) }}
...
{{-- モーダル --}}
											<div class="remodal" data-remodal-id="modal">
												<div class="modalTitle">
													<h2>申し込みを承認して<br>以下のメッセージを送ります<br>よろしいですか？</h2>
												</div><!-- /.modalTitle -->
