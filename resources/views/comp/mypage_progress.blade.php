					...
					<p class="tab_btn msgMenu__tab"><a href="/comp/msg/event/list">イベント</a><span class="msgMenu__badge">{{ $member_act['event_cnt'] }}</span></p>
						...
						<div class="secContentsInner">
							...
							<div class="contentsTitle-container" style="display: flex;justify-content: space-between;">
								...
								<ul class="linkList">
									<li><a href="/comp/clientend">終了した候補者一覧</a></li>
									<li><a href="/comp/client/enter">採用者一覧</a></li>
								</ul><!-- /.item -->
							</div>
...
@else
							<table class="tbl-mypage-progress" id="beingTable">
								...
								@foreach ($beingList as $int)
									<tr>
										{{ html()->form('GET', '/comp/interview/flow')->attribute('name', 'userform' . $int->id)->open() }}
										{{ html()->hidden('interview_id', $int->id) }}
										{{ html()->form()->close() }}
                                
										{{ html()->form('POST', '/comp/mypage/progress/list')->attribute('name', 'beingform' . $int->id)->open() }}
										{{ html()->hidden('interview_id', $int->id) }}
...
@else
							<table class="tbl-mypage-progress2 mb-ajust" id="alreadyTable">
								...
								@foreach ($alreadyList as $int)
									<tr>
										{{ html()->form('GET', '/comp/interview/flow')->attribute('name', 'aluserform' . $int->id)->open() }}
										{{ html()->hidden('interview_id', $int->id) }}
										{{ html()->form()->close() }}
                                
										{{ html()->form('POST', '/comp/mypage/progress/list')->attribute('name', 'alreadyform' . $int->id)->open() }}
										{{ html()->hidden('interview_id', $int->id) }}
