...
@if(!isset($beingList[0]))
  <div>※データはありません。</div>
@else
                           <table class="tbl-user-progress" id="beingTable">
                                <tr>
                                    <th>最終更新日</th>
                                    <th>氏名</th>
                                    <th>ステージ</th>
                              ...
                              @foreach ($beingList as $int)
								...
								{{ Form::open(['url' => '/comp/client/list', 'name' => 'beingform' . $int->id ]) }}
                                {{ Form::hidden('interview_id', $int->id) }}
...
@if (!isset($alreadyList[0]))
  <div>※データはありません。</div>
@else
                            <table class="tbl-8th mb-ajust" id="alreadyTable">
                                <tr>
                                    <th>面談日</th>
                                    <th>氏名</th>
                                    <th>ステージ</th>
                              ...
                              @foreach ($alreadyList as $int)
                                ...
                                {{ Form::open(['url' => '/comp/client/list', 'name' => 'alreadyform' . $int->id ]) }}
                                {{ Form::hidden('interview_id', $int->id) }}
                                    <td>
