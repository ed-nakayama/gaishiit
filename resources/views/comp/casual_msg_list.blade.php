						...
						<div class="panel_area">
							...
							<table class="messageBoxTbl" id="msgTable">
								@foreach ($interviewList as $int)
									<tr>
										<th>{{ str_replace(' ','/', str_replace('-','/', substr($int->last_update, 0 ,16))) }}　　　{{ $int->getKind() }}　No.{{ $int->id }}</th>
										<th>
											@if ($int->read_flag == '0')
										...
										<th class="receiveName">
											{{ html()->form('GET', '/comp/interview/flow')->id('intform' . $int->id)->attribute('name', 'intform' . $int->id)->open() }}
