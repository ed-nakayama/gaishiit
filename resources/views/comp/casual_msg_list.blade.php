			...
			<div class="containerContents">
				<section class="secContents-mb">
					<div class="tab_box_no">
						<div class="btn_area">
							<p class="tab_btn msgMenu__tab active"><a href="/comp/msg/casual/list">カジュアル面談</a><span class="msgMenu__badge">{{ $member_act['user_casual_cnt'] }}</span></p>
						...
						<div class="panel_area">
							<p style="text-align: center;">全{{ $interviewList->total() }}件中 @if ($interviewList->total() == 0)0 @else{{  ($interviewList->currentPage() -1) * $interviewList->perPage() + 1}}@endif-{{ (($interviewList->currentPage() -1) * $interviewList->perPage() + 1) + (count($interviewList) -1)  }}件</p>
							<br>
							<table class="messageBoxTbl" id="msgTable">
								@foreach ($interviewList as $interview)
									<tr>
										<th>{{ str_replace(' ','/', str_replace('-','/', substr($interview->last_update, 0 ,16))) }}　　　{{ $interview->getKind() }}　No.{{$interview->id }}</th>
										<th>
										...
										<th>
											@if ($interview->interview_kind == '1')
												{{ $interview->unit->name }}
												@elseif  ($interview->interview_kind == '2')
												<a href="/comp/job/ref/{{  $interview->job->id }}" target="_blank" style="text-decoration: underline;">{{ $interview->job->name }}</a>
											@else
