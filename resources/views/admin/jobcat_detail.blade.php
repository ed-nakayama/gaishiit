			...
			<div class="secContentsInner">
						...
						<ul class="item-btn">
							...
							<li style="width: auto;" >
								<div class="selectWrap">
									{{ html()->form('POST', '/admin/jobcatdetail')->attribute('name', 'changeform')->open() }}
							...
							@if (auth()->user()->cat_priv == '1')
								<li style="width: 300px;">
									{{ html()->form('POST', '/admin/jobcatdetail/add')->id('addform')->attribute('name', 'addform')->open() }}
									{{ html()->hidden('jobcat_id', $job_cat->id) }}
				...
				<table class="tbl-cat-5th mb-ajust" id="memberTable">
					...
					@foreach ($catList as $cat)
						 <tr>
							{{ html()->form('POST', '/admin/jobcatdetail/store')->id('catform' . $cat->id)->attribute('catform' . $cat->id)->open() }}
							{{ html()->hidden('jobcat_id', $job_cat->id) }}
							{{ html()->hidden('jobcatdetail_id', $cat->id) }}
							<td>{{ $cat->id }}</td>
