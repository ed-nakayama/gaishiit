...
<head>
    <title>インダストリ管理 | {{ config('app.name', 'Laravel') }}</title>
</head>
                        ...
                        <div class="secContentsInner">
                            ...
                            <div class="secBtnHead">
                                ...
                                <div class="secBtnHead-btn">
                                    <ul class="item-btn">
                                       ...
                                       <li style="width: 300px;" >
											...
											{{ Form::open(['url' => '/admin/industorycatdetail', 'name' => 'changeform']) }}
                                       ...
                                       <li style="width: 300px;">
	        	                           {{ Form::open(['url' => '/admin/industorycatdetail/add', 'name' => 'addform' , 'id' => 'addform']) }}
			                               {{ Form::hidden('cat_id', $indCat->id) }}
                                       		<input type="text" name="solo_name" value="{{ old('solo_name') }}" placeholder="インダストリ詳細名">
		                                   {{ Form::close() }}
                                       </li>
                             ...
                             <table class="tbl-cat-5th mb-ajust" id="memberTable">
                                ...
                                <tr>
                               {{ Form::open(['url' => '/admin/industorycatdetail/store', 'name' => 'catform' . $cat->id ]) }}
	                           {{ Form::hidden('industorycat_id', $cat->industory_cat_id) }}
                               {{ Form::hidden('industorydetail_id', $cat->id) }}
                                    <td>{{ $cat['id'] }}</td>
                                    <td><input type="text" name="order_num" value="{{ $cat->order_num }}" oninput="インダストリ('{{ 'catsave' . $cat->id  }}')"></td>
                                    <td><input type="text" name="name" value="{{ $cat->name }}" oninput="インダストリ('{{ 'catsave' .$cat->id  }}')"></td>
                                    <td><input type="checkbox" name="del_flag" value="1" @if ($cat->del_flag == '1') checked @endif   onchange="インダストリ('{{ 'catsave' . $cat->id  }}')"></td>
                                    <td>
