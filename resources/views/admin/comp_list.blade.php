					...
					@if (auth()->user()->comp_priv == '1')
						<div class="secBtnHead" style="justify-content: flex-start;">
							<div class="secBtnHead-btn">
								{{ html()->form('GET', '/admin/comp/register')->id('regform')->attribute('name', 'regform')->open() }}
								{{ html()->hidden('comp_id') }}
								<ul class="item-btn">
							...
							<div class="secBtnHead-btn">
								{{ html()->form('GET', '/admin/comp/list')->id('searchform')->attribute('name', 'searchform')->open() }}
...
@else
					...
					<table class="tbl-comp-list mb-ajust">
						<tr>
							<th>企業ID</th><th>企業名</th><th>SFID</th><th>紹介</th><th>公開</th><th>ARK<br>代理管理</th><th>当月<br>InMail数<br>正式応募</th><th><br>カジュアル</th><th>最大<br>InMail数<br>正式応募</th><th><br>カジュアル</th><th></th>
						</tr>
                               
						@foreach ($compList as $comp)
							<tr>
								{{ html()->form('GET', '/admin/comp/edit')->id('editform')->attribute('name', 'editform' . $comp->id)->open() }}
