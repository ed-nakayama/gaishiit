...
@section('addheader')
	<title>企業評価 確認｜{{ config('app.title') }}</title>
	<meta name="description" content="企業評価 確認｜{{ config('app.description') }}">

	<meta property="og:type" content="article" />
	<meta property="og:title" content="企業評価 確認｜{{ config('app.title') }}" />
	<meta property="og:description" content="企業評価 確認｜{{ config('app.description') }}" />
...
<main class="pane-main">
	...
	<div class="inner">
		...
		@foreach ($errors->all() as $error)
		  <span class="invalid-feedback" role="alert" style="color:#ff0000;"><p style="transform: rotate(0.03deg);">{{$error}}</p></span>
		@endforeach

		{{ html()->form('POST', "/eval/send")->id('storeform')->attribute('name', 'storeform')->open() }}
			...
			<div class="item setting">
				<div class="item-inner">
					<div class="setting-list">
						...
						<div class="btn-wrap">
							<p style="font-size: 14px; color: #ff0000;">※ 送信をすると、この企業への評価の編集、削除することはできません。</p>
							<button type="button" style="margin:10px 0;"  onclick="history.back()">戻る</button>　
							<button type="submit" style="margin:10px 0;">送信する</button>
						</div>
