						...
						<div class="form-inner">
							<div class="job-check-box-btn">
								@foreach ($businessCat as $cat)
									<label>
										<a href="/job/list/buscat{{$cat->id}}" style="border-bottom: 1px solid #000;"><b>{{ $cat->name }}</b></a>
									</label><br>
									@foreach ($businessCatDetail as $detail)
										@if ($cat->id == $detail->business_cat_id)
											<label style="margin-bottom:10px;">
												<a href="/job/list/business{{ $detail->id }}"><span>{{ $detail->name }}</span>
											</label>
										@endif
									@endforeach
									<br>
