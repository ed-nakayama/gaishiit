				...
				<div class="content-container">
					...
					@foreach ($businessCat as $cat)
						<div class="content" id="Job{{ $cat->id }}">
							<label>{{ html()->checkbox("buscat_parent[]", 0, $cat->id)->id("buscat_parent")->attribute('title', $cat->name)->class("buscat_parent{$cat->id}") }}<span style="font-weight:bold;">{{ $cat->name }}全て</span></label>
							<hr>
@php
	$totalCnt = 0;
	foreach ($businessCatDetail as $detail) {
		if ($detail->business_cat_id == $cat->id) $totalCnt++;
	}
@endphp
...
@endphp
									@foreach ($businessCatDetail as $detail)
										@if ($detail->business_cat_id == $cat->id)
											@if ($cnt < $totalCnt / 2)
												<li><label>{{ html()->checkbox("buscat_sel[]", 0, $detail->id)->id('buscat_select')->attribute('title', $detail->name)->class("buscat_sel{$cat->id}") }}{{ $detail->name }}</label></li>
											@endif
@php
	$cnt++;
@endphp
