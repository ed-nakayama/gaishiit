			...
			@foreach ($industoryCat as $cat)
				<div id="" class="block">
					<ul class="cate-parent">
						<li>
							<label>
								{{ html()->checkbox("industorycat_parent[]", 0, $cat->id)->id("industorycat_parent")->attribute('title', $cat->name)->class("industorycat_parent{$cat->id}") }}<span>{{ $cat->name }}</span>
							</label>
						</li>
					</ul>
					<p class="block-ttl"></p>
					...
					<ul class="cate-list">
						@foreach ($industoryCatDetail as $detail)
							@if ($detail->industory_cat_id == $cat->id)
								<li>
									<label>
										{{ html()->checkbox("industorycat_sel[]", 0, $detail->id)->id('industorycat_select')->attribute('title', $detail->name)->class("industorycat_sel{$cat->id}") }}
